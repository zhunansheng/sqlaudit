          ...
          <div class="edittable-table-height-con">
            <Form>
                  <FormItem>
                    ...
                    <Button type="warning" @click="check_sql('check')">语法校验</Button>
                    <Button type="success" @click="commit_sql('soar')">优化</Button>
                    <Button type="info" @click="clear_sql">指纹</Button>
                    <Button type="error" @click="clear_sql">清空</Button>
                    <br>
                    <br>
                    <Card v-show="resultshow">
          <p slot="title">
            <Icon type="android-remove"></Icon>
            {{ resultarg }}
          </p>
...
<script>
  ...
  export default {
    ...
    methods: {
            ...
            axios.post(`${util.url}/sqlsoar`, {
              ...
              .then(res => {
                if (res.data.status === 'success') {
                  ...
                  if (arg === 'check') {
                    this.resultarg = '语法校验结果为'
                  } else {
                    this.resultarg = '优化结果为'
                  }
                } else {
                  util.err_notice('后端解析发生错误')
                }
              })
              .catch(error => {
      ...
      canel () {
        this.formItem = this.formItemsrc
